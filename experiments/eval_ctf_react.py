...
from .utils.ollama_api import OllamaCompletion, OllamaChat
...
parser.add_argument('--env', choices=['ctf'], help='Intercode environment to run eval on')
...
class ExperimentWrapper():
    def __init__(self, args):
        ...
        self.env = CTFEnv(f"intercode-ctf", traj_dir="logs/ctf/", data_path="./data/ctf/ic_ctf.json", preprocess=preprocess_ctf, verbose=True)
        self.env.reset()
        ...
        try:
            for idx in tqdm(range(len(self.env.data_loader)), disable=self.args.verbose):
                ...
                for turn in range(1, self.args.max_turns + 1):
                    # Determine thought and action # stop=[f"\nObservation {turn}:"],
                    thought_action = llm(prompt + f"Thought {turn}:", model="phi3")
                    try:
